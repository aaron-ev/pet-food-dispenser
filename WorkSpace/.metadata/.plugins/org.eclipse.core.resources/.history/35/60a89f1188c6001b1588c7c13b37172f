...
void servo_Write(uint8_t degrees)
{
	...
	switch (degrees)
	{
		case SERVO_DEGREE_90: __HAL_TIM_SET_COMPARE(&tim2PWM,TIM_CHANNEL_1,SERVO_SIGNAL_2ms);
				 	 	 	  HAL_Delay(SERVO_DELAY);
				 	 	 	  break;
		case SERVO_DEGREE_0: __HAL_TIM_SET_COMPARE(&tim2PWM,TIM_CHANNEL_1,SERVO_SIGNAL_1ms);
							 HAL_Delay(SERVO_DELAY);
							 break;
